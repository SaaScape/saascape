...
export default (app: Router, use: any) => {
  ...
  router.put('/:id', use(withPerms([permissions.APPLICATIONS.UPDATE_APPLICATIONS])), use(update))
  router.delete('/:id', use(withPerms([permissions.APPLICATIONS.DELETE_APPLICATIONS])), use(deleteOne))
  router.post('/', use(withPerms([permissions.APPLICATIONS.CREATE_APPLICATIONS])), use(create))
...
const updateConfig: API = async (req, res) => {
  const applicationService = new ApplicationService()
  const { application } = (await applicationService.updateConfig(req.params.id, req.body)) || {}
  sendSuccessResponse({ application }, req, res)
}
